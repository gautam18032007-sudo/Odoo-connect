...
// plaintext Neon connection string (including password) as a fallback
// default. Removed entirely — environment configuration only, fail
// closed if DATABASE_URL is missing. Never reintroduce a hardcoded
// credential here, even as a "fallback".
...
if (!process.env.DATABASE_URL) {
  console.error('❌ DATABASE_URL is not set (checked process.env and .env.local). Refusing to run without it — no hardcoded fallback credential exists in this script.');
  process.exit(1);
}
