...
async function main() {
	...
	if (!client.getMockModeStatus()) {
		...
		const rawOdooIds = missingOrderIds.map((o) => Number(String(o.id).replace("pos_", "")));
		...
		console.table(odooOrders.map((o) => ({ id: o.id, name: o.name, linesCount: o.lines?.length })));

		const allLineIds = odooOrders.flatMap((o) => o.lines || []);
		console.log(`\nFetching ${allLineIds.length} POS order line IDs from Odoo:`, allLineIds);

		const odooLines = await client.callKw<any[]>("pos.order.line", "search_read", [], {
		...
		const productIdsInLines = [...new Set(odooLines.map((l) => Array.isArray(l.product_id) ? Number(l.product_id[0]) : Number(l.product_id)))];
		console.log(`\nProduct IDs referenced in lines (${productIdsInLines.length} distinct):`, productIdsInLines);
		...
		const missingProductIds = productIdsInLines.filter((id) => !dbProductIds.has(id));
		console.log(`\nProduct IDs MISSING from dim_products (${missingProductIds.length}):`, missingProductIds);
	}
